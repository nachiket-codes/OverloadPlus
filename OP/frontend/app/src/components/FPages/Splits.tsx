...
import { deleteSplits, getSplits } from "../../features/splitSlice";
...
const Splits = () => {
    ...
    const [openSplitId, setOpenSplitId] = useState<string>("")
    ...
    const deleteSplit = async (e: React.FormEvent, id: string) => {
        e.preventDefault();
        await dispatch(deleteSplits(id))
                        ...
                        (<ul className="flex flex-col gap-2">
                            {
                                splits.map((item, idx) => {
                                    ...
                                    <li className="border-b border-primary p-2 pl-4 hover:border-b-2 flex relative">
                                        <div className="flex-1 cursor-pointer hover:text-primary " onClick={()=>setOpenSplitId(item.id)}>
                                            ...
                                            <p className="text-[12px] text-gray-400 opacity-0">{!item.description && 'placeholder'}</p>
                                        </div>
                ...
                <div>
                    <AnimatePresence>
                        ...
                        )}
                        {
                            openSplitId && (
                                <div>
                                <BackDrop unselectionFunc={()=>setOpenSplitId("")}/>
                                <SplitModal id={openSplitId} closeFunc={()=>setOpenSplitId("")}/>
                                </div>
                            )
                        }
                    </AnimatePresence>
